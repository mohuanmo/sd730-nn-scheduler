...
# 修复:
#   1) layer 匹配: 旧实现 grep "${pkg%.*}" (截掉最后一段) + head -1,
#      对 com.android.* / com.google.* / com.tencent.* 会命中系统层/错误 layer,
#      导致 --latency 拿不到前台 app 的帧 -> 全部跳过。
#      现在: 完整包名固定串匹配 (grep -F), 优先带 # 的 Activity/Surface layer, 逐候选尝试。
#   2) 解析: 旧实现 tail -n +4 硬跳行, 遇到不同表头会错位。
#      现在只取 3 列纯数字行 (层名/刷新周期/表头全部跳过), 更稳。
#   3) 前台 app 切换 -> 清空 .frame_stats (旧 app 的帧统计不能标新 app 的线程)。
#   4) 失败写日志 (不再静默, 便于排查)。
...
get_frame_stats() {
    ...
    [ -z "$list" ] && { log_msg "[frame] SurfaceFlinger --list empty/timeout for $pkg"; return 1; }
